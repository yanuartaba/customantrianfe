...
function EditModalGroup(props) {
  const { showEditModal, toggleEdit, group } = props;
  ...
  const updateGroup = async () => {
    ...
    try {
      await axios.patch(`http://localhost:3001/menus/${group.id}`, payload, {
  ...
  useEffect(() => {
    setCodeGroup(group.codeGroup);
    setLabel(group.label);
    setDeskripsi(group.description);
